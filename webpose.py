...
def start_pos():
    ...
    while True:
        ...
        for i in range(0, detections.shape[2]):
                ...
                if endX-startX>75:
                        print("MOVE BACK",endX-startX)
                        a+=1
                        if a==1:
                                system("say Please move back")
                        os.system(command)
                elif endX-startX<50:
                        print("MOVE FORWARD",endX-startX)
                        b+=1
                        if b==1:
                                system("say Please move forward")
                        os.system(command)
                else:
                        print("GOOD POSTURE",endX-startX)
...
tk.title("WebPOSE")
canvas=Canvas(width=1120,height=630,bg="#fcfcfc")
canvas.pack()
photo=PhotoImage(file="webposelogo.png")
